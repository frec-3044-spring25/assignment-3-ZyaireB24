...
**Answer 1:**
There is an overall postive trend in the CO2 data, as time has progressed the CO2 levels have increased.
**Question 2:**
...
```
During February, the monthly average was at its max for this dataset.

```{r}
co2 |>
  group_by(year, month, monthly_average)|>
  summarise()
...
- Where is the data from?
  the data is from NASA online data base
  
- Describe the class of each column and what units it is measured in. 
   this data set has 3 columns, the first being Year followed by No_smoothing and lowess
   
- What is the difference between "global temperature" and "global temperature anomaly"? 
  How much warmer than average the most recent year was globally
